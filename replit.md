...
- **Market Intelligence Page** (`/market-intelligence`): Bloomberg-style living data report aggregating all published job data. Public page (SEO-friendly). Sections: key stats bar, skills demand (Recharts bar chart), career paths grid, salary insights (Pro-gated beyond top 3), work mode donut chart, AI intensity bars, seniority landscape chart, top hiring companies, geography breakdown, student spotlight, community pulse (Pro-gated, only renders with 5+ diagnostics). All numbers from `/api/market-intelligence` endpoint with 1-hour in-memory cache (`marketIntelligenceCache` in routes.ts, TTL `MI_CACHE_TTL = 3600000`). Recharts charts use CSS variables `--chart-1` through `--chart-5`.
- **Market Intelligence API** (`/api/market-intelligence`): Public endpoint. Returns overview stats, skillsDemand (top 15), careerPaths with newThisWeek, salaryByPath (median min/max, 3+ sample threshold), workMode split, aiIntensity (Low/Med/High using `computeAIIntensity`), seniorityDistribution, topCompanies (top 10), geography (top 15 countries), studentOpportunities summary, and communityBenchmarks (when 5+ diagnostics exist). Cached in-memory for 1 hour.
- **Students Page** (`/students`): Aspirational landing page for law students entering legal tech. Sections: hero with dual CTAs (browse roles + quiz), "Why Legal Tech" value props, live stats strip, featured student opportunities (job cards from API), career paths for students, quiz/diagnostic CTA, law school partnership section (partnership outreach). All data from `/api/student-opportunities` endpoint.
- **Student Opportunities API** (`/api/student-opportunities`): Public endpoint. Returns totalStudentJobs (Intern + Fellowship + Entry + Junior seniority), top 20 recent student-eligible jobs, pathBreakdown by career category, topCompanies (top 10), remoteCount.
