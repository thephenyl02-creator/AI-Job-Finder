...
- **Resume Review Flow** (`/resume-review/:jobId`): Unified resume improvement flow. Upload → Match → Select Job → Review line-by-line suggestions (redline diff: original vs suggested with Accept/Reject per line) → Final resume ready → Apply. Free users see blurred preview of suggestions; Pro users get full Accept/Reject functionality. Replaces old Tailor My Resume dialog. NextStepCard on job detail page and match result cards on jobs page link to this flow.
- **Resume Strategy**: AI-powered recommendations for resume alignment against job postings (legacy dialog, still accessible from job detail page).
