...
- **Market Intelligence Page**: A Bloomberg-style career intelligence terminal (`/market-intelligence`) with data-dense, scannable panels. Sections: Hero, Market Pulse (6-stat grid), Three Paths (Lawyer-Led/Technical/Ecosystem track cards with accessibility labels), Skill Bridge (per-track "You Likely Have" vs "To Build"), Entry Corridors (categories ranked by accessibility score), Skills in Demand (horizontal bar), Salary (custom range bars), Work Mode (CSS stacked bar), AI Intensity (inline metrics), Seniority (horizontal bar), Companies (horizontal bar), Geography (horizontal bar), Transition-Friendly Employers (table), Community Pulse (Pro-gated), Market Evolution (line/bar charts, ≥2 months only), and CTA. Chart types: horizontal bar (ranked data), CSS stacked bar (composition), line chart (time-series ≥2 months). No pie/donut/area charts. Pro users download PDF reports via `GET /api/market-intelligence/report?period=weekly|monthly|annual`. Admin downloads DOCX via `GET /api/admin/market-intelligence/docx`. OG meta tags set dynamically. PDF/DOCX hardcode `lawjobs.co` as production domain.
