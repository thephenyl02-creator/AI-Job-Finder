...
### Core Features
- **Unified Smart Search**: The Jobs page (`/jobs`) features an integrated AI-powered search bar that replaces the basic text filter. Users type natural language queries (e.g., "remote compliance role, 3+ years") and get AI-scored results. Includes guided search flow with clarifying questions inline on the same page. Free users get 7 guided search trials per session (tracked via sessionStorage), then guided search requires Pro. Quick search (skip questions) is always available. The `/search` page still exists as an alternative entry point. Search suggestion chips shown when search bar is empty. Smart empty state with clear-filters and search suggestions when 0 results found.
...
- **ATS Resume Review**: Analyzes uploaded resumes for ATS (Applicant Tracking System) friendliness. Provides overall score, section-by-section analysis (Contact Info, Summary, Experience, Skills, Education, Keywords, Structure), keyword analysis with strong/missing keywords, formatting tips, and prioritized actionable improvements. Available to all authenticated users from the Resumes page. Key files: `client/src/pages/resumes.tsx`, `server/routes.ts` (POST /api/resume/ats-review).
- **Post a Job with File Upload**: Non-admin users can upload PDF/DOCX job descriptions that auto-parse and pre-fill the submission form. Key files: `client/src/pages/post-job.tsx`, `server/routes.ts` (POST /api/parse-job-file).
- **Password Reset Flow**: Forgot password UI with email input, token-based reset (direct token return for self-hosted setup), and success states. Key files: `client/src/pages/auth.tsx`, `server/replit_integrations/auth/routes.ts`.
- **SEO**: Per-page document titles via `usePageTitle` hook, Open Graph and Twitter meta tags in index.html. Key files: `client/src/hooks/use-page-title.ts`, `client/index.html`.
