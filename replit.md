...
- **Quality Assurance**: A comprehensive QA validation system with persisted results and an admin review queue ensures high data integrity. Dashboard market alignment query filters by both `is_active` AND `is_published` for accurate counts. Two-tier rejection system: `ABSOLUTE_REJECT_PATTERNS` (never published regardless of company — medical, clinical, notary, talent pools) and `ALWAYS_REJECT_TITLE_PATTERNS` (bypassed for legal tech companies). 657 published jobs after full data quality audit (30+ junk/non-legal-tech jobs cleaned).
- **Country Normalization**: Deterministic ISO-2 country code extraction for global job location accuracy. All published jobs have valid country codes (zero "UN/Unknown"). Manual fixes applied for 22 jobs with ambiguous locations.
...
- **Events** (`/events`): Curated listing of real, verified legal tech events (conferences, workshops, webinars, CLEs). 11 manually curated events from established organizers (ILTACON, Legalweek, CLOC, Stanford CodeX, ABA, Georgetown, PLI, Legal Hackers, Sedona Conference, etc.). AI event discovery is disabled — all events are manually curated only. Events page has clean empty state, verified registration links, event type badges, attendance format icons, topic tags, and relative time indicators.
...
- **Market Intelligence Page** (`/market-intelligence`): Bloomberg-style living data report aggregating all published job data. Public page (SEO-friendly). Redesigned with colorful metric cards (tinted backgrounds), download report dropdown (weekly/monthly/annual PDF), salary range visualization bars, card-based companies/geography with rank badges. Sections: key stats cards, skills demand (Recharts bar chart with multi-color bars), career paths grid with colored accent strips, salary insights with visual range bars (Pro-gated beyond top 3), work mode donut chart with center total label, AI intensity bars, seniority landscape chart, top hiring companies, geography breakdown, community pulse (Pro-gated, only renders with 5+ diagnostics). All numbers from `/api/market-intelligence` endpoint with 1-hour in-memory cache (`server/lib/mi-cache.ts`, TTL `MI_CACHE_TTL = 3600000`). Recharts charts use CSS variables `--chart-1` through `--chart-5`.
- **Market Intelligence PDF Reports** (`/api/market-intelligence/report`): Public endpoint. Bloomberg Law / McKinsey-quality PDF reports generated via `pdfkit` (`server/lib/market-intelligence-pdf.ts`). PDF version 1.7 for full Adobe Acrobat 64-bit compatibility. Three periods: `?period=weekly|monthly|annual`. 4-page layout: navy cover page with live stat boxes, 2 tightly-packed content pages (numbered sections 01-07 with navy rules, 2x3 executive summary stat boxes, horizontal bar charts for skills/seniority, professional tables with alternating rows for career paths and salary data, side-by-side work mode boxes + AI intensity bars, two-column ranked company/geography lists with mini accent bars), navy methodology back page with data sources and coverage. Proper PDF metadata (Title, Author, Creator). All data from market intelligence cache with defensive null checks. Returns `Content-Type: application/pdf` with `Content-Disposition: attachment`.
