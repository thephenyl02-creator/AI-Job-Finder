...
- **Skill Extraction**: AI categorizer produces `hardSkills` (tools, platforms, languages, certifications, methodologies) and `softSkills` (professional/transferable skills with specific applications). `keySkills` is kept as the combined flat list for backward compatibility.
- **Skill Quality Rules**: Hard skills must name specific tools/platforms (e.g., "Relativity" not "eDiscovery"). Soft skills must describe specific applications (e.g., "Cross-functional stakeholder management" not "Communication").
- **Categorization Boundaries**: Explicit YES/NO boundary examples prevent over-bucketing into Legal Operations and Legal Consulting. Customer Success at vendor → Consulting; CLM roles → Contract Management; eDiscovery tools → Litigation & eDiscovery.
...
- **Market Evolution UI**: The Market Intelligence page includes a "Market Evolution" section with Recharts visualizations: job volume over time (grouped bar), skills trajectory (line chart). Only shown when ≥2 months of data exist; otherwise shows a graceful tracking note. Data sourced from `/api/stats/historical`.
- **PDF Historical Section**: The downloadable PDF report includes a "Market Evolution" section (section 09) with monthly job volume tables, category distribution by month, top skills across all months, and work mode trends.
...
- **Market Intelligence Page**: A Bloomberg-style career intelligence terminal (`/market-intelligence`) with data-dense, scannable panels. Sections: Hero, Market Pulse (6-stat grid), Three Paths (Lawyer-Led/Technical/Ecosystem track cards with accessibility labels), Skill Bridge (per-track "You Likely Have" vs "To Build"), Entry Corridors (categories ranked by accessibility score), Skills in Demand (side-by-side horizontal bars for Tools & Technical vs Professional Skills when hard/soft data available, single chart fallback), Salary (custom range bars), Work Mode (CSS stacked bar), AI Intensity (inline metrics), Seniority (horizontal bar), Companies (horizontal bar), Geography (horizontal bar), Transition-Friendly Employers (table), Community Pulse (Pro-gated), Market Evolution (line/bar charts, ≥2 months only), and CTA. Chart types: horizontal bar (ranked data), CSS stacked bar (composition), line chart (time-series ≥2 months). No pie/donut/area charts. Pro users download PDF reports via `GET /api/market-intelligence/report?period=weekly|monthly|annual`. Admin downloads DOCX via `GET /api/admin/market-intelligence/docx`. OG meta tags set dynamically. PDF/DOCX hardcode `lawjobs.co` as production domain.
- **Transition Intelligence API**: `GET /api/market-intelligence/transition` computes lawyer-specific career data with 5-min cache: track summaries (job count, %, avg relevance, transition-friendly ratio, avg experience, top skills), entry corridors (13 categories ranked by accessibility score 0-100), skill bridge (legal vs non-legal skills per track), transition-friendly employers, and regional intelligence. Accessibility score formula: 25% entry-mid ratio + 30% transition-friendly ratio + 20% experience score + 25% legal relevance.
- **Unified Chart Theme**: `client/src/lib/chart-theme.ts` provides TRACK_COLORS (Lawyer-Led=`--chart-1` navy, Technical=`--chart-2` blue, Ecosystem=`--chart-5` teal), getCategoryColor() (track-derived with opacity steps), GENERIC_PALETTE, STATUS_COLORS (green/amber/red for readiness only), WORK_MODE_PALETTE, SHARED_TOOLTIP_STYLE, SHARED_AXIS_STYLE, and accessibilityLabel(). Bloomberg-style utility classes in `index.css`: `.mi-metric`, `.mi-metric-sm`, `.mi-label`, `.mi-section-title`, `.mi-panel`, `.mi-insight`, `.mi-delta-up/down/flat`.
- **Pro Gating on MI Page**: Free sees top 3 skills per column in Skill Bridge, top 5 Entry Corridors, top 3 Transition Employers. Pro sees all. Contextual upgrade prompts with Lock + Crown icons.
