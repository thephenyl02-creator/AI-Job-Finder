...
### Core Features
- **Landing Page**: Highlights career intelligence. Features an interactive "How It Works" stepped walkthrough (3 steps with auto-advance timer, progress bar, visual previews showing resume parsing, readiness score, and matched roles). Includes a two-row flowing company logo marquee ("Who's Hiring in Legal Tech") powered by `/api/job-density` returning top 15 companies with job counts. Rows scroll in opposite directions (25s normal, 45s reduced-motion) with gradient edge fades and reduced-motion accessibility support.
