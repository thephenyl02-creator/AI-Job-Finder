...
- **Contextual Conversational Touchpoints**: Inline chat sections (`ContextualPrompt` component) embedded on search results, resumes, career advisor, alerts, and insights pages. Each provides page-specific persona-aware suggestion chips (e.g., "What's trending in Legal AI?" on search, "Tailor tips for Compliance roles" on resumes). Collapsible with a full embedded chat experience. Key file: `client/src/components/contextual-prompt.tsx`.
