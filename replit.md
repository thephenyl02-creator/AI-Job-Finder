...
- **Career Command Center Dashboard** visual redesign: Hero metric cards row (Readiness Score ring, Active Jobs count, Top Path, Activity Streak) with tinted backgrounds; "This Week" action plan with visual progress bar and numbered step markers; Market Pulse as individual mini cards in 2-column grid with colored icon backgrounds; enhanced sidebar with engagement card.
- **Market Intelligence Page** (`/market-intelligence`): Bloomberg-style living data report aggregating all published job data. Public page (SEO-friendly). Redesigned with colorful metric cards (tinted backgrounds), download report dropdown (weekly/monthly/annual PDF), salary range visualization bars, card-based companies/geography with rank badges. Sections: key stats cards, skills demand (Recharts bar chart with multi-color bars), career paths grid with colored accent strips, salary insights with visual range bars (Pro-gated beyond top 3), work mode donut chart with center total label, AI intensity bars, seniority landscape chart, top hiring companies, geography breakdown, student spotlight, community pulse (Pro-gated, only renders with 5+ diagnostics). All numbers from `/api/market-intelligence` endpoint with 1-hour in-memory cache (`marketIntelligenceCache` in routes.ts, TTL `MI_CACHE_TTL = 3600000`). Recharts charts use CSS variables `--chart-1` through `--chart-5`.
- **Market Intelligence PDF Reports** (`/api/market-intelligence/report`): Public endpoint. Bloomberg Law / McKinsey-quality PDF reports generated via `pdfkit` (`server/lib/market-intelligence-pdf.ts`). Three periods: `?period=weekly|monthly|annual`. Features: navy cover page with live stats, numbered sections with navy rules, 2x3 executive summary stat boxes, horizontal bar charts for skills/seniority, professional tables with alternating rows for career paths and salary data, work mode + AI intensity visualization, ranked company/geography lists, student spotlight box, methodology back page. All data from market intelligence cache. Returns `Content-Type: application/pdf` with `Content-Disposition: attachment`.
- **Market Intelligence API** (`/api/market-intelligence`): Public endpoint. Returns overview stats, skillsDemand (top 15, with synonym merging and title-case formatting), careerPaths with newThisWeek, salaryByPath (median min/max, 3+ sample threshold), workMode split, aiIntensity (Low/Med/High using `computeAIIntensity`), seniorityDistribution, topCompanies (top 10), geography (top 15 countries), studentOpportunities summary, and communityBenchmarks (when 5+ diagnostics exist). Cached in-memory for 1 hour. **Skills synonym merging**: Maps duplicates to canonical names (e.g. "legal tech" → "legal technology", "customer engagement" → "client engagement"). **Title-case formatting**: Proper casing with acronym handling (AI, ML, API, CRM, etc.).
- **Students Page** (`/students`): Landing page for law students entering legal tech (deferred for expansion — current inventory ~20 genuine student roles after proper filtering). Sections: hero with dual CTAs (browse roles + quiz), "Why Legal Tech" value props, live stats strip, featured student opportunities (job cards from API), career paths for students, quiz/diagnostic CTA, law school partnership section. All data from `/api/student-opportunities` endpoint.
- **Student Opportunities API** (`/api/student-opportunities`): Public endpoint. Uses `isStudentRole()` filter: always includes Intern/Fellowship seniority; for Entry/Junior, only includes if title contains student-signaling keywords (intern, summer, fellow, graduate, campus, trainee, analyst, etc.); always excludes professional titles (manager, director, senior, lead, etc.). Returns totalStudentJobs, top 20 recent jobs, pathBreakdown, topCompanies (top 10), remoteCount. The same `isStudentRole()` function is used in the market intelligence `studentOpportunities` section for consistency.
