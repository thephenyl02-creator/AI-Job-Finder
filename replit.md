...
- **Resume Review Flow** (`/resume-review/:jobId`): THE single resume improvement path. Upload → Match → Review line-by-line suggestions (redline diff: original vs suggested with Accept/Reject per line) → Final resume ready → Apply. Free users see blurred preview of suggestions; Pro users get full Accept/Reject functionality. All old overlapping dialogs (ResumeRewriteDialog, ResumeStrategyDialog) have been removed. NextStepCard on job detail page and inline match results link to this flow.
- **Resume Strategy**: Removed — consolidated into Resume Review flow to eliminate overlapping features.
...
- **Resume Editor** (`/resume-editor/:resumeId?jobId=X&mode=my|model`): Full-featured inline editor for tailoring resumes to specific jobs. 5 AI agents (Intake, RequirementMapping, ModelResume, HonestyVerifier, Export) process resume + job, produce structured sections with suggestion diffs. ContentEditable inline editing, undo/redo (Ctrl+Z/Y), debounced autosave (800ms), version conflict detection, Requirements Drawer with coverage indicators, To Confirm queue for unverified claims, Ready to Apply status badge. Exports: PDF (PDFKit), DOCX (docx lib), Apply Pack ZIP (Pro only, includes cover letter via AI). Schema: `resumeEditorVersions` table with JSON sections, requirement mapping, version numbers, honesty verification results. All routes have ownership validation (resume must belong to user).
