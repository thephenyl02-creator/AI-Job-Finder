...
- **Country Normalization**: `server/lib/country-normalizer.ts` — deterministic ISO-2 country code extraction from location strings. Covers 100+ cities, 50+ countries. Wired into `createJob` and `upsertJobByExternalId` so all new/updated jobs get countryCode, countryName, workMode automatically. Backfill script: `server/scripts/backfill-countries.ts`.
- **API**: Paginated `/api/jobs` endpoint with server-side filtering (category, seniority, location, search, country, workMode). GET `/api/job-density` aggregates published jobs by country for the Opportunity Map.
...
- **Resume Review Flow**: DEPRECATED — replaced by Resume Editor. Old `/resume-review/:id` URLs redirect to `/jobs/:id`. ResumePickerDialog on job detail and jobs pages opens Resume Editor flow.
- **Resume Strategy**: Removed — consolidated into Resume Editor flow to eliminate overlapping features.
- **Opportunity Map** (`/opportunity-map`): Free global discovery tool showing SVG world choropleth map colored by job density per country. Uses react-simple-maps with Natural Earth 110m TopoJSON. Intelligence panel shows total jobs, country count, remote share, top categories, top companies — updates on hover. Click any country to navigate to filtered jobs. Fully responsive with mobile-optimized stacked layout.
...
- **Resume Editor** (`/resume-editor/:resumeId?jobId=X`): AI-powered full-rewrite editor for tailoring resumes to specific jobs. Single unified rewrite-agent replaces previous tailoring/model agents. Flow: Intake parses resume → Rewrite agent rewrites summary, all bullets, and skills in one pass → Editor shows "read and review" experience with auto-applied changes. Changed items highlighted with subtle border; click to see rewrite reason, original text, and revert option. EditorBullet/EditorSkill track originalText, rewriteReason, grounded, reverted, addedByAI flags. Per-item revert (reverted=true → export uses originalText) or bulk "Revert All". Staged loading with 4 progress messages. ContentEditable inline editing, undo/redo (Ctrl+Z/Y), debounced autosave (800ms), version conflict detection. Exports: PDF (PDFKit), DOCX (docx lib), Apply Pack ZIP (Pro only, includes cover letter via AI). Post-export toast with "Apply Now" and "Tailor for another role" actions. Tailored versions listed on /resumes page via GET /api/resumes/tailored-versions. Schema: `resumeEditorVersions` table with JSON sections, version numbers. All routes have ownership validation (resume must belong to user).
