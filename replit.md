...
### Core Features
- **Landing Page**: 3-section design inspired by Claude.ai. Section 1: Hero with headline, CTA, trust signals, live intelligence ticker (cycles real data from `/api/market-pulse`, `/api/stats`, `/api/job-density` every 3s), and a premium free-floating animated composition (6 panels, no bounding box). Panels: resume parsing (scan-line + parsing state), **Career Diagnostic centerpiece** (largest panel, shadow-xl, score ring 0→72 count-up, 3 career path bars, 3 skill gap pills), Market Intelligence (animated SVG donut chart with real workModeSplit data + trending skill), job matches (fit % count-up via rAF), tailor notification (68%→91% animated color shift), apply badge. Each panel uses nested wrappers (entrance animation + `animate-gentle-float` drift at different phases). 6.5s choreographed timeline, plays once and holds. Section 2: Company marquee (15s normal, 30s reduced-motion) + career path floating badges on dotted grid. Section 3: Final CTA with journey step icons (Profile → Path → Jobs → Tailor → Apply). Removed: social proof strip, stats strip, How It Works walkthrough, standalone career paths, global coverage section, Free vs Pro comparison.
