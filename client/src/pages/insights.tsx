...
const SENIORITY_ORDER = ["Entry", "Mid", "Senior", "Lead", "Director", "VP"];
...
export default function Insights() {
  ...
  const maxCompanyCount = topCompanies[0]?.jobCount || 1;

  const sortedSeniority = [...seniorityBreakdown].sort(
    (a, b) => SENIORITY_ORDER.indexOf(a.level) - SENIORITY_ORDER.indexOf(b.level)
  );

  const totalExpJobs = experienceRanges.entry + experienceRanges.mid + experienceRanges.senior + experienceRanges.expert;
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground max-w-xl">
            Real-time statistics from our legal technology job database. Use these numbers to understand where the opportunities are.
          </p>
        </div>
